...
Admin image uploads are stored locally in the `uploads_data` Docker volume and served from `/uploads/`. No Cloudinary account is required. Keep this volume when rebuilding containers, and include it in the VPS backup plan. Uploaded filenames are generated by the API, validated by file content, and are not committed to Git.
...
Back up uploaded images separately from PostgreSQL:

```bash
docker compose --env-file .env.production exec -T api tar czf - -C /app/uploads . > uploads-backup.tar.gz
```
