...
type AdminRecord={
  ...
  productPlacements?:RelationId[];guidePlacements?:RelationId[];commerceCategoryPlacements?:RelationId[];
};
...
const modules=["Overview","SEO Audit","Contact Messages",...Object.keys(configs),"Settings"];
type Options=Record<string,{id:string;name?:string;title?:string;label?:string}[]>;
...
export default function Admin(){
  ...
  const visibleModules=useMemo(()=>modules.filter(module=>currentAdmin?.role!=="EDITOR"||!["Contact Messages","Users / Admins","Settings"].includes(module)),[currentAdmin?.role]);
...
function fieldPlaceholder(field:Field){
  ...
  const named:Record<string,string>={name:"e.g., Example Store",title:"e.g., Best kitchen deals this week",slug:"e.g., example-store",websiteUrl:"https://www.example.com",affiliateUrl:"https://retailer.example/your-link",affiliateBaseUrl:"https://retailer.example/affiliate",color:"e.g., #ff6413",ctaLabel:"e.g., Shop now",ctaText:"e.g., View deals",ctaUrl:"e.g., /store/example-store",currentPrice:"e.g., 49.99",oldPrice:"e.g., 79.99",discountPercent:"e.g., 20",sku:"e.g., ABC-123",tags:"e.g., Editor’s pick, Travel",faqItems:'[{"question":"…","answer":"…"}]',importantSpecs:'[{"label":"…","value":"…"}]',config:'{"key":"value"}',links:'[{"label":"About","url":"/about"}]'};
  if(named[field.name])return named[field.name];
...
function FieldInput({field,value,options,uploading,onImage,onChange}:{field:Field;value:unknown;options:Options;uploading:boolean;onImage:(file?:File)=>void;onChange:(value:unknown)=>void}){
  ...
  if(field.type==="textarea"||field.type==="lines"||field.type==="json"){
    ...
    return <textarea required={field.required} value={textValue} placeholder={fieldPlaceholder(field)} onChange={event=>onChange(event.target.value)} rows={field.name==="content"?10:4}/>;
  }
  ...
  if(field.type==="multiselect"){const selected=Array.isArray(value)?value.map(String):[];return <select multiple value={selected} onChange={event=>onChange(Array.from(event.target.selectedOptions,option=>option.value))}>{field.option&&options[field.option]?.map(option=><option value={option.id} key={option.id}>{option.name||option.title||option.label}</option>)}</select>}
...
function normalize(row:AdminRecord,config:Config){const value:AdminRecord={...row};if(config.resource==="homepage-sections"){value.productIds=relationIds(row.products,"productId");value.categoryIds=relationIds(row.categories,"categoryId");value.storeIds=relationIds(row.stores,"storeId")}if(config.resource==="categories")value.featuredProductIds=relationIds(row.featuredProducts,"productId");if(config.resource==="blog"){value.productIds=relationIds(row.productPlacements,"productId");value.guideIds=relationIds(row.guidePlacements,"guideId");value.commerceCategoryIds=relationIds(row.commerceCategoryPlacements,"categoryId")}if(config.resource==="guides")value.productIds=relationIds(row.products,"productId");if(config.resource==="comparisons"){value.productIds=relationIds(row.products,"productId");value.storeIds=relationIds(row.stores,"storeId")}if(Array.isArray(value.tags))value.tags=value.tags.map(String).join(", ");for(const key of ["keyFeatures","pros","cons","bestUseCases","buyingTips","shoppingTips","expertise","profileUrls"])if(Array.isArray(value[key]))value[key]=value[key].map(String).join("\n");return value}
function icon(module:string){return ({Overview:"⌂","SEO Audit":"✓","Contact Messages":"✉",Products:"◇",Categories:"▦",Stores:"▣",Brands:"◎",Deals:"%",Banners:"▤","Homepage Sections":"▥",Blog:"✎",Authors:"◉","Buying Guides":"▧",Comparisons:"⇄","Trust Pages":"§",Navigation:"☷",Footer:"▬","Affiliate Links":"↗","Users / Admins":"♙",Settings:"⚙"} as Record<string,string>)[module]||"◇"}
