...
type Field={name:string;label:string;type?:"text"|"email"|"textarea"|"richtext"|"lines"|"number"|"url"|"date"|"checkbox"|"select"|"multiselect"|"image"|"images"|"json";required?:boolean;option?:string;choices?:string[];help?:string;placeholder?:string};
...
function RecordModal({config,initial,options,busy,onClose,onSave}:{config:Config;initial:AdminRecord;options:Options;busy:boolean;onClose:()=>void;onSave:(value:AdminRecord)=>void}){
  ...
  return <div className="modal-backdrop"><form className="record-modal" role="dialog" aria-modal="true" aria-labelledby="record-modal-title" onSubmit={submit}><header><div><small>{initial.id?"EDIT":"CREATE"}</small><h2 id="record-modal-title">{config.singular}</h2></div><button type="button" onClick={onClose} aria-label={`Close ${config.singular} form`}>×</button></header><div className="form-intro"><strong>Quick guide</strong><p>Start with fields marked <b>Required</b>. Everything else is optional and can be added later. Short tips and examples are shown under each field.</p></div><div className="form-grid">{config.fields.map(field=>{const help=fieldHelp(field);const className=field.type==="textarea"||field.type==="richtext"||field.type==="lines"||field.type==="json"||field.type==="images"?"wide":"";const input=<FieldInput field={field} value={values[field.name]} options={options} uploading={uploading===field.name} onImage={file=>image(field,file)} onChange={value=>setValues(current=>({...current,[field.name]:value}))}/>;const heading=field.type!=="checkbox"&&<span className="field-label"><b>{field.label}{field.required&&<em>Required</em>}</b>{help&&<small>{help}</small>}</span>;return field.type==="multiselect"||field.type==="richtext"?<div key={field.name} className={`form-field ${className}`}>{heading}{input}</div>:<label key={field.name} className={className}>{heading}{input}{field.type==="checkbox"&&help&&<small className="field-help">{help}</small>}</label>})}</div><footer><button className="cancel" type="button" onClick={onClose}>Cancel</button><button className="save" type="submit" disabled={busy||Boolean(uploading)}>{busy?"Saving…":initial.id?"Save changes":"Create"}</button></footer></form></div>
}
...
function RichTextEditor({field,value,onChange}:{field:Field;value:string;onChange:(value:unknown)=>void}){
  const textarea=useRef<HTMLTextAreaElement>(null);
  function replaceSelection(transform:(selected:string)=>string){const input=textarea.current;if(!input)return;const start=input.selectionStart;const end=input.selectionEnd;const replacement=transform(value.slice(start,end));onChange(`${value.slice(0,start)}${replacement}${value.slice(end)}`);window.setTimeout(()=>{input.focus();input.setSelectionRange(start,start+replacement.length)},0)}
  function prefixLines(prefix:(index:number)=>string){replaceSelection(selected=>(selected||"List item").split("\n").map((line,index)=>`${prefix(index)}${line.replace(/^([-*+]\s+|\d+[.)]\s+|[a-zA-Z][.)]\s+)/,"")}`).join("\n"))}
  function addLink(){const url=window.prompt("Paste an internal path or complete link","");if(!url)return;const safe=isSafeProductLink(url);if(!safe)return window.alert("Use an internal path beginning with / or a complete http(s) URL.");replaceSelection(selected=>`[${selected||"Link text"}](${url.trim()})`)}
  return <div className="rich-editor">
    <div className="rich-editor-toolbar" role="toolbar" aria-label="Blog formatting">
      <button type="button" onClick={()=>replaceSelection(selected=>`### ${selected||"Section heading"}`)}>H3</button>
      <button type="button" onClick={()=>replaceSelection(selected=>`**${selected||"Important text"}**`)}><strong>Bold</strong></button>
      <button type="button" onClick={()=>prefixLines(()=>"- ")}>• Bullets</button>
      <button type="button" onClick={()=>prefixLines(index=>`${index+1}) `)}>1. Numbered</button>
      <button type="button" onClick={()=>prefixLines(index=>`${String.fromCharCode(97+index)}) `)}>a. Lettered</button>
      <button type="button" onClick={addLink}>🔗 Link</button>
      <button type="button" onClick={()=>replaceSelection(()=>"| Item | Details |\n| --- | --- |\n| Example | Add information |")}>▦ Table</button>
    </div>
    <textarea ref={textarea} required={field.required} value={value} placeholder="Paste or write the complete article here…" onPaste={event=>pasteStructuredBlogContent(event,value,onChange)} onChange={event=>onChange(event.target.value)} rows={16}/>
    <small className="rich-editor-note">Formatting is saved with the article. You can paste formatted content directly from a document or webpage.</small>
  </div>
}
function FieldInput({field,value,options,uploading,onImage,onChange}:{field:Field;value:unknown;options:Options;uploading:boolean;onImage:(file?:File)=>void;onChange:(value:unknown)=>void}){
  ...
  if(field.type==="richtext"){const textValue=typeof value==="string"?value:"";return <RichTextEditor field={field} value={textValue} onChange={onChange}/>}
  if(field.type==="textarea"||field.type==="lines"||field.type==="json"){
    ...
    return <textarea required={field.required} value={textValue} placeholder={fieldPlaceholder(field)} onChange={event=>onChange(event.target.value)} rows={4}/>;
  }
...
function inlineMarkdown(node:Node):string{
  if(node.nodeType===Node.TEXT_NODE)return (node.textContent||"").replace(/\s+/g," ");if(!(node instanceof HTMLElement))return "";
  const content=Array.from(node.childNodes).map(inlineMarkdown).join("").trim();const tag=node.tagName;
  if(tag==="BR")return "\n";if((tag==="STRONG"||tag==="B")&&content)return `**${content}**`;if((tag==="EM"||tag==="I")&&content)return `*${content}*`;
  if(tag==="A"&&content){const href=node.getAttribute("href")||"";return isSafeProductLink(href)?`[${content}](${href})`:content}return content;
}
function tableMarkdown(table:HTMLTableElement){
  const rows=Array.from(table.rows).map(row=>Array.from(row.cells).map(cell=>inlineMarkdown(cell).replaceAll("|","\\|").trim()));if(!rows.length)return "";const width=Math.max(...rows.map(row=>row.length));const normalized=rows.map(row=>Array.from({length:width},(_,index)=>row[index]||""));const header=normalized[0];return [`| ${header.join(" | ")} |`,`| ${header.map(()=>"---").join(" | ")} |`,...normalized.slice(1).map(row=>`| ${row.join(" | ")} |`)].join("\n");
}
function htmlBlocks(root:HTMLElement):string[]{
  const output:string[]=[];for(const node of Array.from(root.children)){const element=node as HTMLElement;const tag=element.tagName;
    if(/^H[1-6]$/.test(tag)){const text=inlineMarkdown(element);if(text)output.push(`### ${text}`);continue}
    if(tag==="UL"||tag==="OL"){const lettered=tag==="OL"&&(element.getAttribute("type")||"").toLowerCase()==="a";const items=Array.from(element.children).filter(child=>child.tagName==="LI").map((child,index)=>`${tag==="UL"?"-":lettered?`${String.fromCharCode(97+index)})`:`${index+1})`} ${inlineMarkdown(child)}`);if(items.length)output.push(items.join("\n"));continue}
    if(tag==="TABLE"){const table=tableMarkdown(element as HTMLTableElement);if(table)output.push(table);continue}
    if(tag==="BLOCKQUOTE"){const text=inlineMarkdown(element);if(text)output.push(text.split("\n").map(line=>`> ${line}`).join("\n"));continue}
    if(tag==="P"||tag==="DIV"){const hasBlockChild=Array.from(element.children).some(child=>["P","DIV","UL","OL","TABLE","BLOCKQUOTE","H1","H2","H3","H4","H5","H6"].includes(child.tagName));if(hasBlockChild)output.push(...htmlBlocks(element));else{const text=inlineMarkdown(element);if(text)output.push(text)}continue}
    const text=inlineMarkdown(element);if(text)output.push(text)
  }return output;
}
function pasteStructuredBlogContent(event:ClipboardEvent<HTMLTextAreaElement>,current:string,onChange:(value:unknown)=>void){
  ...
  const documentValue=new DOMParser().parseFromString(html,"text/html");const structured=htmlBlocks(documentValue.body).filter(Boolean).join("\n\n");
