...
type Post={id:string;slug:string;title:string;excerpt?:string|null;content:string;coverImage?:string|null;author?:string|null;authorProfile?:Author|null;publishedAt?:string|null;contentUpdatedAt?:string|null;updatedAt:string;readingTime?:number|null;category?:{name:string;slug:string}|null;faqItems?:unknown;relatedProducts?:any[];relatedGuides?:any[];relatedCategories?:{id:string;name:string;slug:string}[];seoTitle?:string|null;seoDescription?:string|null;canonicalUrl?:string|null;ogTitle?:string|null;ogDescription?:string|null;ogImage?:string|null;robotsIndex?:boolean;robotsFollow?:boolean;schemaEnabled?:boolean};
...
export async function generateMetadata({params,searchParams}:Props):Promise<Metadata>{
  ...
  const metadata=entityMetadata({...post,robotsIndex:post.robotsIndex!==false&&Object.keys(query).length===0},{title:post.title,description:post.excerpt||post.content.slice(0,160),path:`/blog/${post.slug}`,image:post.coverImage},settings as any);
  return {...metadata,openGraph:{...metadata.openGraph,type:"article",publishedTime:post.publishedAt||undefined,modifiedTime:post.contentUpdatedAt||post.updatedAt,authors:post.authorProfile?[absoluteUrl(`/authors/${post.authorProfile.slug}`,settings as any)]:undefined}};
}
...
export default async function BlogPostPage({params}:Props){
  ...
  const schema=post.schemaEnabled!==false?{"@context":"https://schema.org","@type":"BlogPosting",headline:post.title,description:post.excerpt||undefined,image:post.coverImage?absoluteUrl(post.coverImage,settings as any):undefined,datePublished:post.publishedAt||undefined,dateModified:updated,author:visibleAuthor?{"@type":"Person",name:visibleAuthor,url:post.authorProfile?absoluteUrl(`/authors/${post.authorProfile.slug}`,settings as any):undefined}:undefined,mainEntityOfPage:absoluteUrl(post.canonicalUrl||`/blog/${post.slug}`,settings as any)}:null;
  return <PublicShell><article className={`${styles.page} ${styles.narrow}`}>
    <Breadcrumbs items={[{name:"Home",url:"/"},{name:"Blog",url:"/blog"},{name:post.title,url:`/blog/${post.slug}`}]} settings={settings as any} schema={post.schemaEnabled!==false}/>
    ...
    <AffiliateDisclosure text={(settings as any).affiliateDisclosure}/>
    <div className={styles.body}><RichText text={post.content}/></div>
    <FaqList items={faqs}/>
