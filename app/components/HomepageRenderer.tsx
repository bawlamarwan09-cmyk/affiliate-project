...
function Hero({ section, disclosure }: { section?: HomepageSection; disclosure?: string | null }) {
  if (!section) return <section className="reference-hero quiet-empty"><div className="hero-disclosure"><AffiliateDisclosure text={disclosure}/></div><div className="hero-copy"><span className="eyebrow">New deals added regularly</span><h1>Big Deals.<br/><em>Smart Shopping.</em></h1><p>Discover worthwhile offers from trusted online stores, all gathered in one easy place.</p><form className="hero-search" action="/search"><input name="q" aria-label="Search" placeholder="Search products, brands or categories"/><button>Search deals</button></form><div className="hero-benefits"><span>★ Helpful product context</span><span>◇ Clear price information</span><span>◆ Store links</span><span>▣ Secure redirects</span></div></div><div className="hero-art"><SafeImage className="hero-shopping-photo" src="/brand/hero-shopping-cutout.webp" alt="Woman holding colorful shopping bags" width={960} height={640} sizes="(max-width: 800px) 100vw, 50vw" priority/></div></section>;
  return <section className="reference-hero cms-hero" style={{ background: section.background }}><div className="hero-disclosure"><AffiliateDisclosure text={disclosure}/></div><div className="hero-copy"><span className="eyebrow">{String(section.config?.eyebrow || "")}</span><h1>{section.title}</h1><p>{section.subtitle}</p><form className="hero-search" action="/search"><input name="q" aria-label="Search" placeholder={String(section.config?.searchPlaceholder || "Search products, brands or categories")}/><button>{section.ctaText || "Search deals"}</button></form>{Array.isArray(section.config?.benefits) && <div className="hero-benefits">{(section.config.benefits as string[]).map((value) => <span key={value}>◆ {value}</span>)}</div>}</div><div className="hero-art">{section.config?.image ? <SafeImage src={String(section.config.image)} alt={String(section.config.imageAlt || section.title || "Shopping deals")} width={720} height={520} sizes="(max-width: 800px) 100vw, 50vw" priority/> : <SafeImage className="hero-shopping-photo" src="/brand/hero-shopping-cutout.webp" alt="Woman holding colorful shopping bags" width={960} height={640} sizes="(max-width: 800px) 100vw, 50vw" priority/>}</div></section>;
}
