...
const productReferenceUrl=z.string().trim().min(1).refine(value=>safeInternalPath(value)||safeHttpUrl(value),"Use a safe internal product path or an absolute http(s) URL");
...
const defaultDealEnd=()=>new Date("2099-12-31T23:59:59.999Z");
...
const schemas={
  ...
  deals:z.object({...base,slug:z.string().min(2).regex(/^[a-z0-9-]+$/),productId:z.string().min(1),discountPercent:z.coerce.number().int().min(0).max(100).nullable().optional(),startsAt:optionalDate,endsAt:optionalDate,status:status.default("DRAFT"),featured:z.coerce.boolean().default(false),badge:optionalString,...seoFields,robotsIndex:z.coerce.boolean().default(false),schemaEnabled:z.coerce.boolean().default(false)}).refine(v=>!v.startsAt||!v.endsAt||v.endsAt>v.startsAt,{message:"End date must be after start date",path:["endsAt"]}),
...
function sanitize(resource:Resource,raw:unknown,id?:string){
  ...
  if(resource==="deals"){
    const now=new Date();
    const end=parsed.endsAt instanceof Date?parsed.endsAt:null;
    if(!(parsed.startsAt instanceof Date)||(parsed.status==="ACTIVE"&&end&&end<=now))parsed.startsAt=now;
    if(!end||(parsed.status==="ACTIVE"&&end<=now))parsed.endsAt=defaultDealEnd();
  }
